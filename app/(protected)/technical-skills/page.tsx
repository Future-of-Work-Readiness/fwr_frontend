...
import { Loader2, Code, ChevronRight, ArrowLeft, CheckCircle2, AlertCircle, Clock } from "lucide-react";
 ...
 */
function LevelStatusBadge({ levelSummary }: { levelSummary: LevelAttemptSummary | undefined }) {
  if (!levelSummary || !levelSummary.attempted) {
    return null;
  }
            ...
            {!hasSpecialisation ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {exercisesByLevel.map((levelData, levelIndex) => {
                  const levelSummary = attemptSummary?.levels[String(levelData.levelNumber)];
                  ...
                  return (
                    <ScrollReveal key={levelData.level} delay={levelIndex * 0.1}>
                      <Card className={`h-full flex flex-col hover:shadow-xl transition-all duration-300 border-2 bg-white group ${
                        isPassed 
                        ...
                        <CardHeader className="flex-1 pb-3">
                          ...
                          <CardDescription className="text-sm">
                            {levelData.questionCount} questions • {levelData.totalDuration} mins
                          </CardDescription>
                              ...
                              {levelSummary.lastAttemptAt && (
                                <div className="flex items-center gap-1 text-muted-foreground">
                                  ...
                                  <span className="text-xs">
                                    Last taken: {formatRelativeTime(levelSummary.lastAttemptAt)}
                                  </span>
                                </div>
                              )}
                              {!isPassed && (
