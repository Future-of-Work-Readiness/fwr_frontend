 ...
 * Allows components to react to auth changes without prop drilling.
 */
...
export interface AuthEventPayloads {
  ...
  'auth:token-refreshed': { success: boolean };
}
...
/**
 * Subscribe to a single auth event
 */
...
/**
 * Subscribe to multiple auth events at once
 */
export function subscribeToAuthEvents(
  ...
  (
    Object.entries(handlers) as [AuthEventType, (detail: unknown) => void][]
