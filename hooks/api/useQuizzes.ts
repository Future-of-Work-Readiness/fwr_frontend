 ...
 */
function transformQuizHistoryItem(item: QuizAttemptHistoryBackend): QuizAttemptHistory {
  ...
  const specializationName = item.specializationName || item.specialization_name;
  const completedAt = item.completedAt || item.completed_at || '';
  const timeTakenMinutes = item.timeTakenMinutes ?? item.time_taken_minutes;
  const timeTakenSeconds = item.timeTaken ?? item.time_taken_seconds ?? (timeTakenMinutes ? timeTakenMinutes * 60 : 0);
 ...
 */
function transformQuizHistoryResponse(response: QuizHistoryResponseBackend): QuizHistoryResponse {
  return {
...
export function useQuizHistoryQuery(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const url = queryString ? `/quizzes/history?${queryString}` : '/quizzes/history';
...
export function useSaveQuizProgress() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      sessionStorage.setItem(`quiz_progress_${data.attemptId}`, JSON.stringify(data));
    },
    onError: (error) => {
...
export function useQuizProgressQuery(
  attemptId: string,
  options?: Omit<UseQueryOptions<QuizProgress, ApiError>, 'queryKey' | 'queryFn'>
) {
  return useQuery({
